...
class XCParser:
    ...
    def _parse_statement(self) -> Optional[ASTNode]:
        ...
        elif self._match(TokenType.IDENT) and self.current.value == "typedef":
            # 兼容“接近 C 替代”阶段：typedef 行暂按原生声明跳过语义化，
            # 可结合 ⟨T ...⟩ 直通到 C 输出。
            while not self._match(TokenType.SEMICOLON, TokenType.EOF, TokenType.RBRACE):
                self._advance()
            if self._match(TokenType.SEMICOLON):
                self._advance()
            return None
